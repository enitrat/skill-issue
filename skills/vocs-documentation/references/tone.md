...
Brevity has a second payoff on Vocs: your prose is served verbatim to agents through `.md` routes
and `llms-full.txt`. Padding a human skims past is context an agent spends budget on.

## Page Titles and Subtext

Every page opens with `# Title [Subtext]`. Vocs renders the bracketed text as page subtext and uses
it to fill in `title` and `description` metadata — which is exactly what appears in `llms.txt`.

```
GOOD: # useBalance [Fetch the native currency balance of an address]
GOOD: # Authentication [Sign users in and manage sessions]
BAD:  # useBalance                      (no description anywhere)
BAD:  # Overview [Overview of the SDK]  (says nothing)
```

Subtext is a description, not a restatement of the title. Write it as a capability: what the reader
can do after this page.

Section headings (`##`) should be **task-based** for the same reason: they are the unit an agent
retrieves and cites. "Handle Errors" beats "Errors"; "Configure Retries" beats "Notes".
...
Use Vocs' callout directives for information that deserves visual emphasis. Full syntax in
[vocs.md → Callouts](vocs.md#callouts-admonitions).
...
| `:::info` / `:::note` | Reassurance or context | "Not ready to migrate yet? The v1 docs are still available at..." |
| `:::tip` | Helpful side-information | "TypeScript doesn't support importing JSON as const yet. Check out the CLI!" |
| `:::danger` | Destructive or security-critical behavior | "This permanently deletes the resource." |
| `:::success` | Confirming a completed tutorial | "Your SDK is now configured and ready to use." |
| `:::details` | Optional deeper dives | TypeScript configuration, advanced patterns |

Use the bracketed-title form when the default label isn't specific enough:
`:::warning[Breaking change in v3]`.

Never write a warning as bold prose inside a paragraph — it loses its emphasis in the rendered page
and disappears entirely into surrounding text in the Markdown output.
...
```
# [Concept Name] [One-line framing of the idea]

[1-2 paragraph overview: what this concept is and why it matters]

## How it works
[Mental model — how pieces fit together. Mermaid diagrams welcome.]
