...
```
Real backend (Anvil/local API)  →  receives actual RPC calls
Mock connector (wallet sim)     →  simulates auth, proxies data calls to real backend
TanStack Query                  →  exercises full cache lifecycle
React (Chromium via Playwright) →  real browser rendering for hook tests
```
...
export default async function () {
  ...
  return async () => { await mainnet.stop(); await optimism.stop() }
}
```
...
```ts
// 1. Connection state — disconnect any active connector
beforeEach(async () => { await disconnect(config, { connector }) })

// 2. TanStack Query cache — clear before each hook render
queryClient.clear()  // inside custom renderHook wrapper

// 3. Backend state — reset to initial fork after all tests in a suite
afterAll(async () => { await testClient.restart() })
```

### Mock Connector

Not a pure mock — simulates connection layer but proxies data calls:
...
test('select data', () => {
  const result = useBalance({
    query: {
      select(data) { return data?.value },  // Balance -> bigint
    },
  })
