...
### GitBook vs VitePress — feature comparison

| Feature | VitePress | GitBook |
|---|---|---|
| Code groups / tabs | `::: code-group` | `{% tabs %}` |
| Line focus | `// [!code focus]` | Not available (use comments instead) |
| Diff annotations | `// [!code ++]` / `// [!code --]` | Not available (use separate before/after blocks) |
| Admonitions | `::: warning` / `::: tip` | `{% hint style="warning" %}` |
| Expandable | `::: details` | `<details><summary>` |
| File inclusion | `<<< @/snippets/file.ts` | `{% include "path" %}` (reusable content) |
| Template variables | `<script setup>` + `{{ }}` | Not available |
| TypeScript hover | `twoslash` | Not available |
| Step-by-step | Manual headings | `{% stepper %}` (native) |
| OpenAPI rendering | Requires plugin | Native `{% openapi %}` block |
| Interactive API testing | Requires external tool | Built-in "Test it" panel |
| File attachments | Not built-in | `{% file %}` block |
| Embeds | Manual iframe | `{% embed url="..." %}` |
| Git Sync | Native (source of truth) | Bidirectional with GitHub/GitLab |

### What GitBook does better for SDKs

1. **OpenAPI integration** — first-class spec rendering with `x-codeSamples` for SDK examples
2. **Stepper blocks** — native step-by-step UI for tutorials (no manual heading numbering)
3. **AI-powered search** — built-in semantic search across all docs
4. **Interactive embeds** — StackBlitz/CodeSandbox demos via `{% embed %}`
5. **No build step** — Git push → live docs (no CI pipeline to maintain)

### What GitBook lacks vs VitePress

1. **No line annotations** — can't focus, highlight, or diff individual lines in code
2. **No template variables** — can't interpolate package names/types across shared content
3. **No twoslash** — no TypeScript hover annotations in code blocks
4. **No conditional rendering** — can't show/hide content based on variables
5. **Reusable content is Pro-only** — free tier can't use shared includes
